<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __('Assurez-vous que votre compte utilise un mot de passe long et complexe pour rester sécurisé') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            <x-primary-button class="bg-marron">{{ __('Sauvegarder') }}</x-primary-button>
            ...
            @if (session('status') === 'password-updated')
                ...
                >{{ __('Votre mot de passe a bien été modifié.') }}</p>
            @endif
        </div>
    </form>
</section>
